...
public:
    ...
    inline void operator()(T& t, serializer& ser)
    {
        ...
        if constexpr ( std::is_pointer_v<T> ) {
            ...
            if constexpr ( std::is_class_v<typename std::remove_pointer<T>::type> && !std::is_polymorphic_v<typename std::remove_pointer<T>::type> ) {
        ...
        if constexpr ( std::is_pointer_v<T> ) {
            ...
            if constexpr ( std::is_class_v<typename std::remove_pointer<T>::type> && !std::is_polymorphic_v<typename std::remove_pointer<T>::type> ) {
                ...
                if ( ser.mode() == serializer::MAP ) {
                    ...
                    if ( nullptr == t ) return;
                    SST::Core::Serialization::ObjectMap* map = new SST::Core::Serialization::ObjectMapClass(t, typeid(T).name());
...
public:
    ...
    inline void operator()(T& t, serializer& ser, const char* name)
    {
        return serialize_impl<T>()(t, ser, name); }
