...
doGraphOutput(SST::Config* cfg, ConfigGraph* graph)
{
    ...
    if ( cfg->output_config_graph != "" ) {
        // Uncomment once we have a flag for parallel output
        // if ( cfg.output_config_graph.length() != 0 ) {
        //     // Append rank number to base filename
        //     int index = cfg.output_config_graph.find_last_of(".");
        //     std::string base = cfg.output_config_graph.substr(0,index);
        //     std::string ext = cfg.output_config_graph.substr(index);
        //     cfg.output_config_graph = base + std::to_string(myRank.rank) + ext;
        // }
...
main(int argc, char* argv[])
{
    ...
    if ( myRank.rank == 0 || cfg.parallel_load ) {
        try {
            graph = modelGen->createConfigGraph(myRank);
        }
    ...
    if ( !cfg.parallel_load ) {
        ...
        //       hash        ID
        // std::map<uint32_t, LinkId_t> clash_check;
        ...
        for ( ConfigLinkMap_t::iterator iter = links.begin(); iter != links.end(); ++iter ) {
            ...
            if ( rank[0].rank == rank[1].rank || ( rank[0].rank != myRank.rank && rank[1].rank != myRank.rank) ) continue;
            ...
            if ( !ret.second ) {
                g_output.fatal(CALL_INFO, 1, "ERROR: Found clash in hashes for two link names that cross rank partitions.\n");
            }

            printf("id = %" PRIu32 ", hash = %" PRIu32 " (%" PRIx32 ")\n", clink.id, hash, hash);
