...
Config::insertOptions()
{
    ...
    DEF_ARG("output-directory", 0, "DIR", "Directory into which all SST output files should reside",
        output_directory_, true, false, false);
    DEF_ARG("output-prefix-core", 0, "STR", "set the SST::Output prefix for the core", output_core_prefix_, true,
        true, true);
    ...
    DEF_ARG("timebase", 0, "TIMEBASE", "Set the base time step of the simulation (default: 1ps)", timeBase_, true,
        true, true);
#ifdef SST_CONFIG_HAVE_MPI
...
#endif
    DEF_ARG("timeVortex", 0, "MODULE", "Select TimeVortex implementation <lib.timevortex>", timeVortex_, true, true,
        false);
    ...
    DEF_FLAG("force-rank-seq-startup", 0,
        "Force startup phases of simulation to execute one rank at a time for debug purposes", rank_seq_startup_,
        false, false, true);
    ...
    DEF_ARG("sigusr1", 0, "MODULE", "Select handler for SIGUSR1 signal. See extended help for detail.", sigusr1_,
        true, true, false);
    DEF_ARG("sigusr2", 0, "MODULE", "Select handler for SIGUSR2 signal. See extended help for detail.", sigusr2_,
        true, true, false);
