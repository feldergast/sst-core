...
Config::insertOptions()
{
    ...
    DEF_ARG("model-options", 0, "STR",
        ...
        std::bind(&ConfigHelper::setModelOptions, this, _1), false, false, true);
    ...
    DEF_ARG("stop-at", 0, "TIME", "Set time at which simulation will end execution",
        std::bind(&ConfigHelper::setStopAt, this, _1), true, true, false);
    DEF_ARG("exit-after", 0, "TIME",
        ...
        "formats).",
        std::bind(&ConfigHelper::setExitAfter, this, _1), true, false, false);
    DEF_ARG("partitioner", 0, "PARTITIONER", "Select the partitioner to be used. <lib.partitionerName>",
        std::bind(&ConfigHelper::setPartitioner, this, _1), true, true, false);
    DEF_ARG("heartbeat-period", 0, "PERIOD",
        ...
        std::bind(&ConfigHelper::setHeartbeatSimPeriod, this, _1), true, true, false);
    DEF_ARG("heartbeat-wall-period", 0, "PERIOD",
        ...
        std::bind(&ConfigHelper::setHeartbeatWallPeriod, this, _1), true, true, false);
    ...
    DEF_ARG("output-directory", 0, "DIR", "Directory into which all SST output files should reside",
        std::bind(&ConfigHelper::setOutputDir, this, _1), true, false, false);
    DEF_ARG("output-prefix-core", 0, "STR", "set the SST::Output prefix for the core",
        std::bind(&ConfigHelper::setOutputPrefix, this, _1), true, true, true);
    ...
    DEF_ARG("output-config", 0, "FILE", "File to write SST configuration (in Python format)",
        std::bind(&ConfigHelper::setWriteConfig, this, _1), true, false, true);
    DEF_ARG("output-json", 0, "FILE", "File to write SST configuration graph (in JSON format)",
        std::bind(&ConfigHelper::setWriteJSON, this, _1), true, false, true);
#ifdef SST_CONFIG_HAVE_MPI
    DEF_FLAG_OPTVAL("parallel-output", 0,
        ...
        std::bind(&ConfigHelper::enableParallelOutput, this, _1), true, false, true);
#endif
    ...
    DEF_ARG("output-dot", 0, "FILE", "File to write SST configuration graph (in GraphViz format)",
        std::bind(&ConfigHelper::setWriteDot, this, _1), true, false, true);
    DEF_ARG("dot-verbosity", 0, "INT", "Amount of detail to include in the dot graph output",
        std::bind(&ConfigHelper::setDotVerbosity, this, _1), true, false, true);
    DEF_ARG_OPTVAL("output-partition", 0, "FILE",
        ...
        std::bind(&ConfigHelper::setWritePartitionFile, this, _1), true, false, false);

    /* Advanced Features */
    DEF_SECTION_HEADING("Advanced Options");
    DEF_ARG_EH("timebase", 0, "TIMEBASE", "Set the base time step of the simulation (default: 1ps)",
        std::bind(&ConfigHelper::setTimebase, this, _1), std::bind(&ConfigHelper::getTimebaseExtHelp), true, true,
        true);
#ifdef SST_CONFIG_HAVE_MPI
    DEF_ARG_OPTVAL("parallel-load", 0, "MODE",
        ...
        std::bind(&ConfigHelper::enableParallelLoadMode, this, _1), false, false, true);
#endif
    DEF_ARG("timeVortex", 0, "MODULE", "Select TimeVortex implementation <lib.timevortex>",
        std::bind(&ConfigHelper::setTimeVortex, this, _1), true, true, false);
    ...
    DEF_FLAG_OPTVAL("cache-align-mempools", 0, "[EXPERIMENTAL] Set whether mempool allocations are cache aligned",
        std::bind(&ConfigHelper::setCacheAlignMempools, this, _1), true, true, true);
#endif
    DEF_ARG("debug-file", 0, "FILE", "File where debug output will go",
        std::bind(&ConfigHelper::setDebugFile, this, _1), true, false, true);
    ...
    DEF_ARG_EH("enable-profiling", 0, "POINTS",
        ...
        "points to enable.",
        std::bind(&ConfigHelper::enableProfiling, this, _1), std::bind(&ConfigHelper::getProfilingExtHelp), true, false,
        false);
    DEF_ARG("profiling-output", 0, "FILE", "Set output location for profiling data [stdout (default) or a filename]",
        std::bind(&ConfigHelper::setProfilingOutput, this, _1), true, false, false);
    ...
    DEF_ARG("run-mode", 0, "MODE", "Set run mode [ init | run | both (default)]",
        std::bind(&ConfigHelper::setRunMode, this, _1), true, false, true);
    DEF_ARG("interactive-console", 0, "ACTION",
        ...
        std::bind(&ConfigHelper::setInteractiveConsole, this, _1), true, false, false);
    DEF_ARG_OPTVAL("interactive-start", 0, "TIME",
        ...
        "for parallel runs.",
        std::bind(&ConfigHelper::setInteractiveStartTime, this, _1), true, false, false);
#ifdef USE_MEMPOOL
    DEF_ARG("output-undeleted-events", 0, "FILE",
        ...
        std::bind(&ConfigHelper::setWriteUndeleted, this, _1), true, false, false);
#endif
    DEF_FLAG("force-rank-seq-startup", 0,
        "Force startup phases of simulation to execute one rank at a time for debug purposes",
        std::bind(&ConfigHelper::forceRankSeqStartup, this, _1), false, false, true);
    ...
    DEF_FLAG("disable-signal-handlers", 0, "Disable signal handlers",
        std::bind(&ConfigHelper::disableSigHandlers, this, _1), false, false, false);
    DEF_ARG_EH("sigusr1", 0, "MODULE", "Select handler for SIGUSR1 signal. See extended help for detail.",
        std::bind(&ConfigHelper::setSigUsr1, this, _1), std::bind(&ConfigHelper::getSignalExtHelp), true, true, false);
    DEF_ARG_EH("sigusr2", 0, "MODULE", "Select handler for SIGUSR2 signal. See extended help for detail.",
        std::bind(&ConfigHelper::setSigUsr2, this, _1), std::bind(&ConfigHelper::getSignalExtHelp), true, true, false);
    DEF_ARG_EH("sigalrm", 0, "MODULE",
        ...
        std::bind(&ConfigHelper::setSigAlrm, this, _1), std::bind(&ConfigHelper::getSignalExtHelp), true, true, false);
    ...
    DEF_ARG("checkpoint-wall-period", 0, "PERIOD",
        ...
        "the following formats supported: H:M:S, M:S, S, Hh, Mm, Ss (capital letters are the appropriate numbers for "
        "that value, lower case letters represent the units and are required for those formats.).",
        std::bind(&ConfigHelper::setCheckpointWallPeriod, this, _1), true, false, false);
    DEF_ARG("checkpoint-period", 0, "PERIOD",
        ...
        std::bind(&ConfigHelper::setCheckpointSimPeriod, this, _1), true, false, false);
    ...
    DEF_FLAG("load-checkpoint", 0,
        ...
        std::bind(&ConfigHelper::setLoadFromCheckpoint, this, _1), false, false, false);
    ...
    DEF_ARG_EH("checkpoint-name-format", 0, "FORMAT",
        ...
        std::bind(&ConfigHelper::getCheckpointNameFormatExtHelp), true, false, false);
...
void
Config::insertOptions2()
{
    using namespace std::placeholders;
    // Informational options
    DEF_SECTION_HEADING2("Informational Options");
    DEF_FLAG2("usage", 'h', "Print usage information.", usage_);
    DEF_ARG2("help", 0, "option", "Print extended help information for requested option.", help_, false);
    DEF_FLAG2("version", 'V', "Print SST Release Version", version_);

    /* Basic Options */
    DEF_SECTION_HEADING2("Basic Options (most commonly used)");
    // addVerboseOptions(true);
    DEF_ARG2("num-threads", 'n', "NUM", "Number of parallel threads to use per rank", num_threads2_, true);
    DEF_ARG2("sdl-file", 0, "FILE",
        "Specify SST Configuration file.  Note: this is most often done by just specifying the file without an option."
        " For runs specifying --load-checkpoint, the SDL file is the checkpoint manifest file (.sstcpt file)",
        configFile2_, false);
    DEF_ARG2("model-options", 0, "STR",
        "Provide options to the python configuration script.  Additionally, any arguments provided after a final '-- ' "
        "will be appended to the model options (or used as the model options if --model-options was not specified).",
        model_options2_, false, false, true);
    DEF_FLAG_OPTVAL2("print-timing-info", 0, "Print SST timing information", print_timing2_, true);
    DEF_ARG2("stop-at", 0, "TIME", "Set time at which simulation will end execution", stop_at2_, true, true, false);
    DEF_ARG2("exit-after", 0, "TIME",
        "Set the maximum wall time after which simulation will end execution.  Time is specified in hours, minutes and "
        "seconds, with the following formats supported: H:M:S, M:S, S, Hh, Mm, Ss (capital letters are the "
        "appropriate numbers for that value, lower case letters represent the units and are required for those "
        "formats).",
        exit_after2_, true, false, false);
    DEF_ARG2("partitioner", 0, "PARTITIONER", "Select the partitioner to be used. <lib.partitionerName>", partitioner2_,
        true, true, false);
    DEF_ARG2("heartbeat-period", 0, "PERIOD",
        "Set time for heartbeats to be published (these are approximate timings measured in simulation time, published "
        "by the core, to update on progress)",
        heartbeat_sim_period2_, true, true, false);
    DEF_ARG2("heartbeat-wall-period", 0, "PERIOD",
        "Set approximate frequency for heartbeats (SST-Core progress updates) to be published in terms of wall (real) "
        "time. PERIOD can be specified in hours, minutes, and seconds with "
        "the following formats supported: H:M:S, M:S, S, Hh, Mm, Ss (capital letters are the appropriate numbers for "
        "that value, lower case letters represent the units and are required for those formats.).",
        heartbeat_wall_period2_, true, true, false);
    DEF_ARG2("heartbeat-sim-period", 0, "PERIOD",
        "Set approximate frequency for heartbeats (SST-Core progress updates) to be published in terms of simulated "
        "time. PERIOD must include time units (s or Hz) and SI prefixes are accepted.",
        heartbeat_sim_period2_, true, true, false);
    DEF_ARG2("output-directory", 0, "DIR", "Directory into which all SST output files should reside",
        output_directory2_, true, false, false);
    DEF_ARG2("output-prefix-core", 0, "STR", "set the SST::Output prefix for the core", output_core_prefix2_, true,
        true, true);

    /* Configuration Output */
    DEF_SECTION_HEADING2(
        "Configuration Output Options (generates a file that can be used as input for reproducing a run)");
    DEF_ARG2("output-config", 0, "FILE", "File to write SST configuration (in Python format)", output_config_graph2_,
        true, false, true);
    DEF_ARG2("output-json", 0, "FILE", "File to write SST configuration graph (in JSON format)", output_json2_, true,
        false, true);
#ifdef SST_CONFIG_HAVE_MPI
    DEF_FLAG_OPTVAL2("parallel-output", 0,
        "Enable parallel output of configuration information.  This option is ignored for single rank jobs.  Must also "
        "specify an output type (--output-config and/or --output-json).  Note: this will also cause partition info to "
        "be output if set to true.",
        parallel_output2_, true, false, true);
#endif

    /* Configuration Output */
    DEF_SECTION_HEADING2("Graph Output Options (for outputting graph information for visualization or inspection)");
    DEF_ARG2("output-dot", 0, "FILE", "File to write SST configuration graph (in GraphViz format)", output_dot2_, true,
        false, true);
    DEF_ARG2("dot-verbosity", 0, "INT", "Amount of detail to include in the dot graph output", dot_verbosity2_, true,
        false, true);
    DEF_ARG_OPTVAL2("output-partition", 0, "FILE",
        "File to write SST component partitioning information.  When used without an argument and in conjuction with "
        "--output-json or --output-config options, will cause paritition information to be added to graph output.",
        output_partition2_, true, false, false);

    /* Advanced Features */
    DEF_SECTION_HEADING2("Advanced Options");
    DEF_ARG2("timebase", 0, "TIMEBASE", "Set the base time step of the simulation (default: 1ps)", timeBase2_, true,
        true, true);
#ifdef SST_CONFIG_HAVE_MPI
    DEF_ARG_OPTVAL2("parallel-load", 0, "MODE",
        "Enable parallel loading of configuration. This option is ignored for single rank jobs.  Optional mode "
        "parameters are NONE, SINGLE and MULTI (default).  If NONE is specified, parallel-load is turned off. If "
        "SINGLE is specified, the same file will be passed to all MPI ranks.  If MULTI is specified, each MPI rank is "
        "required to have it's own file to load. Note, not all input "
        "formats support both types of file loading.",
        parallel_load2_, false, false, true);
#endif
    DEF_ARG2("timeVortex", 0, "MODULE", "Select TimeVortex implementation <lib.timevortex>", timeVortex2_, true, true,
        false);
    DEF_FLAG_OPTVAL2("interthread-links", 0, "[EXPERIMENTAL] Set whether or not interthread links should be used",
        interthread_links2_, true);
#ifdef USE_MEMPOOL
    DEF_FLAG_OPTVAL2("cache-align-mempools", 0, "[EXPERIMENTAL] Set whether mempool allocations are cache aligned",
        cache_align_mempools2_, true, true, true);
#endif
    DEF_ARG2("debug-file", 0, "FILE", "File where debug output will go", debugFile2_, true, false, true);
    addLibraryPathOptions2();

#if PY_MINOR_VERSION >= 9
    DEF_FLAG2("enable-python-coverage", 0,
        "[EXPERIMENTAL] Causes the base Python interpreter to activate the coverage.Coverage object. This option can "
        "also be turned "
        "on by setting the environment variable SST_CONFIG_PYTHON_COVERAGE to true.",
        enable_python_coverage2_, false, false, true);
#endif

    /* Advanced Features - Profiling */
    DEF_SECTION_HEADING2("Advanced Options - Profiling (API Not Yet Final)");
    DEF_ARG2("enable-profiling", 0, "POINTS",
        "Enables default profiling for the specified points.  Argument is a semicolon separated list specifying the "
        "points to enable.",
        enabled_profiling2_, true, false, false);
    DEF_ARG2("profiling-output", 0, "FILE", "Set output location for profiling data [stdout (default) or a filename]",
        profiling_output2_, true, false, false);

    /* Advanced Features - Debug */
    DEF_SECTION_HEADING2("Advanced Options - Debug");
    DEF_ARG2("run-mode", 0, "MODE", "Set run mode [ init | run | both (default)]", runMode2_, true, false, true);
    DEF_ARG2("interactive-console", 0, "ACTION",
        "[EXPERIMENTAL] Set console to use for interactive mode. NOTE: This currently only works for serial jobs and "
        "this option will be ignored for parallel runs.",
        interactive_console2_, true, false, false);
    DEF_ARG_OPTVAL2("interactive-start", 0, "TIME",
        "[EXPERIMENTAL] Drop into interactive mode at specified simulated time.  If no time is specified, or the time "
        "is 0, then it will "
        "drop into interactive mode before any events are processed in the main run loop. This option is ignored if no "
        "interactive console was set. NOTE: This currently only works for serial jobs and this option will be ignored "
        "for parallel runs.",
        interactive_start_time2_, true, false, false);
#ifdef USE_MEMPOOL
    DEF_ARG2("output-undeleted-events", 0, "FILE",
        "file to write information about all undeleted events at the end of simulation (STDOUT and STDERR can be used "
        "to output to console)",
        event_dump_file2_, true, false, false);
#endif
    DEF_FLAG2("force-rank-seq-startup", 0,
        "Force startup phases of simulation to execute one rank at a time for debug purposes", rank_seq_startup2_,
        false, false, true);

    /* Advanced Features - Environment Setup/Reporting */
    DEF_SECTION_HEADING2("Advanced Options - Environment Setup/Reporting");
    addEnvironmentOptions2();
    DEF_FLAG2("disable-signal-handlers", 0, "Disable signal handlers", enable_sig_handling2_, false, false, false);
    DEF_ARG2("sigusr1", 0, "MODULE", "Select handler for SIGUSR1 signal. See extended help for detail.", sigusr12_,
        true, true, false);
    DEF_ARG2("sigusr2", 0, "MODULE", "Select handler for SIGUSR2 signal. See extended help for detail.", sigusr22_,
        true, true, false);
    DEF_ARG2("sigalrm", 0, "MODULE",
        "Select handler for SIGALRM signals.  Argument is a semicolon separated list specifying the "
        "handlers to register along with a time interval for each. See extended help for detail.",
        sigalrm2_, true, true, false);

    /* Advanced Features - Checkpoint */
    DEF_SECTION_HEADING2("Advanced Options - Checkpointing (EXPERIMENTAL)");
    DEF_ARG2("checkpoint-wall-period", 0, "PERIOD",
        "Set approximate frequency for checkpoints to be generated in terms of wall (real) time. PERIOD can be "
        "specified in hours, minutes, and seconds with "
        "the following formats supported: H:M:S, M:S, S, Hh, Mm, Ss (capital letters are the appropriate numbers for "
        "that value, lower case letters represent the units and are required for those formats.).",
        checkpoint_wall_period2_, true, false, false);
    DEF_ARG2("checkpoint-period", 0, "PERIOD",
        "Set approximate frequency for checkpoints to be generated in terms of simulated time. PERIOD must include "
        "time units (s or Hz) and SI prefixes are accepted. This flag will eventually be removed in favor of "
        "--checkpoint-sim-period",
        checkpoint_sim_period2_, true, false, false);
    DEF_ARG2("checkpoint-sim-period", 0, "PERIOD",
        "Set approximate frequency for checkpoints to be generated in terms of simulated time. PERIOD must include "
        "time units (s or Hz) and SI prefixes are accepted.",
        checkpoint_sim_period2_, true, false, false);
    DEF_FLAG2("load-checkpoint", 0,
        "Load checkpoint and continue simulation. Specified SDL file will be used as the checkpoint file.",
        load_from_checkpoint2_, false, false, false);
    DEF_ARG2("checkpoint-prefix", 0, "PREFIX",
        "Set prefix for checkpoint filenames. The checkpoint prefix defaults to checkpoint if this option is not set "
        "and checkpointing is enabled.",
        checkpoint_prefix2_, true);
    DEF_ARG2("checkpoint-name-format", 0, "FORMAT",
        "Set the format for checkpoint filenames. See extended help for format options.  Default is "
        "\"%p_%n_%t/%p_%n_%t\"",
        checkpoint_name_format2_, true, false, false);

    //     enableDashDashSupport(std::bind(&ConfigHelper::setModelOptions, this, _1));
    enableDashDashSupport(std::bind(&OptionDefinition::parse, &model_options2_, _1));
    addPositionalCallback(std::bind(&Config::positionalCallback, this, _1, _2));
};
